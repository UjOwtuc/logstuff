...
impl ParseRule {
    fn rule(&self) -> Rule {
        use ParseRule::*;
        match self {
            Query => Rule::bool_expr,
            Field => Rule::field,
            Scalar => Rule::scalar_value,
            List => Rule::list_value,
        }
    }
}
...
pub fn try_parse(input_type: ParseRule, s: &str) -> Result<(), usize> {
    match QueryParser::parse(input_type.rule(), s) {
        ...
        Err(err) => match err.location {
            pest::error::InputLocation::Pos(pos) => Err(pos),
            pest::error::InputLocation::Span((start, _)) => Err(start),
        },
    }
}
